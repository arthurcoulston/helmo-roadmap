...
/* Vendors the estate's crew avatar sprite into src/estate-avatars.generated.ts
   ...
   Same seam as scripts/vendor-estate-tokens.mjs and for the same reason: the
   roadmap is published standalone and stays free of estate imports, so it
   copies the asset in rather than reaching for a sibling checkout at runtime.
...
export function render(svg) {
  ...
  return [
    ...
    '// shares (R-11); the roadmap consumes it as a copy so it stays publishable',
    '// alone. Hues come from the vendored token file — a mark is currentColor',
...
function run() {
  ...
  try {
    ...
    console.error(`no estate avatar sprite at ${SOURCE} — set ESTATE_AVATARS_SVG or clone the estate repo alongside helmo-roadmap`);
    process.exit(2);
  }
